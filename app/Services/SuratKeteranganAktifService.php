...
class SuratKeteranganAktifService
{
    ...
    public function generateDocument(SuratKeteranganAktifApplication $application, ?User $finalApprover = null): string
    {
        ...
        if (!$application->nomor_surat) {
            throw new RuntimeException('Nomor surat belum tersedia untuk dokumen Surat Keterangan Aktif.');
        }

        if (!$this->signatoryService->officialKadepForApplication($application)) {
            throw new RuntimeException('Dokumen Surat Keterangan Aktif tidak dapat dibuat: belum ada Ketua Departemen aktif untuk program studi mahasiswa.');
        }

        $directory = 'surat-keterangan-aktif/generated';
        ...
        $filename = sprintf(
            'surat-keterangan-aktif-%d-%s-%s.pdf',
            $application->id,
            now()->format('YmdHis'),
            Str::random(8)
        );
        ...
        try {
            ...
            if (!Storage::disk('public')->exists($tempPath)) {
                throw new RuntimeException('Gagal menyimpan dokumen sementara PDF Surat Keterangan Aktif.');
            }

            if (!Storage::disk('public')->move($tempPath, $path)) {
                throw new RuntimeException('Gagal memindahkan dokumen PDF Surat Keterangan Aktif.');
            }
            $finalFileCreated = true;

            if (!Storage::disk('public')->exists($path)) {
                throw new RuntimeException('Gagal menyimpan dokumen PDF Surat Keterangan Aktif.');
            }
    ...
    public function generatedPdfStoragePath(SuratKeteranganAktifApplication $application): ?string
    {
        $path = $this->publicDiskPath($application->generated_pdf_path);

        if (!$path || !str_starts_with($path, 'surat-keterangan-aktif/generated/')) {
            return null;
        }
    ...
    private function buildDocumentHtml(SuratKeteranganAktifApplication $application, ?User $finalApprover): string
    {
...
<body>
    ...
    <div class="meta">
        <table>
            ...
            <tr><td class="label">Hal</td><td class="colon">:</td><td>Surat Keterangan Aktif</td></tr>
    ...
    <div class="data">
        <table>
            <tr><td class="label">Nama Orang Tua/Wali</td><td class="colon">:</td><td>' . $this->escape($application->nama_orang_tua_wali) . '</td></tr>
            <tr><td class="label">Pekerjaan</td><td class="colon">:</td><td>' . $this->escape($application->pekerjaan_orang_tua_wali) . '</td></tr>
            <tr><td class="label">NIP</td><td class="colon">:</td><td>' . $this->escape($application->nip_orang_tua_wali) . '</td></tr>
            <tr><td class="label">Pangkat/Golongan</td><td class="colon">:</td><td>' . $this->escape($application->pangkat_gol_orang_tua_wali) . '</td></tr>
            <tr><td class="label">Instansi</td><td class="colon">:</td><td>' . $this->escape($application->instansi_orang_tua_wali) . '</td></tr>
        </table>
    </div>

    <div class="body-copy">
        <p>
            Mahasiswa tersebut benar tercatat aktif pada program studi di lingkungan fakultas/departemen tersebut.
            Surat keterangan ini dibuat untuk keperluan: ' . $this->escape($application->keperluan) . '.
        </p>
