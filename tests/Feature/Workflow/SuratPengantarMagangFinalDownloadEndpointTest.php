...
class SuratPengantarMagangFinalDownloadEndpointTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_owner_can_download_completed_final_pdf_from_latest_ready_mahasiswa_artifact_without_side_effects(): void
    {
        $this->bindServicesThatMustNotRun();
    ...
    public function test_non_owner_cannot_download_final_pdf(): void
    {
        ...
        $this->actingAs($otherStudent, 'sanctum')
            ->getJson("/api/mahasiswa/surat-pengantar-magang/{$application->id}/final-download")
            ->assertForbidden()
    ...
    public function test_owner_cannot_download_before_completed(): void
    {
        ...
        $this->assertSame($artifactCount, LetterDocumentArtifact::query()->count());
        $this->assertSame(SuratPengantarMagangApplication::STATUS_READY_FOR_STUDENT_REVIEW, $application->fresh()->status);
    }

    public function test_completed_application_with_no_artifact_returns_unavailable_without_path_leak(): void
    {
        [$student] = $this->completeMahasiswa();
        $application = $this->completedApplication($student);
    ...
    public function test_generating_latest_mahasiswa_artifact_returns_conflict_without_ready_fallback(): void
    {
        [$student] = $this->completeMahasiswa();
        $application = $this->completedApplication($student);
        $this->artifact($application, LetterDocumentArtifact::PHASE_MAHASISWA_REVIEW, bytes: "%PDF-1.4\nready");
    ...
    public function test_failed_latest_mahasiswa_artifact_returns_service_unavailable_without_ready_fallback(): void
    {
        [$student] = $this->completeMahasiswa();
        $application = $this->completedApplication($student);
        $this->artifact($application, LetterDocumentArtifact::PHASE_MAHASISWA_REVIEW, bytes: "%PDF-1.4\nready");
     ...
     */
    private function completedApplication($student, array $attributes = []): SuratPengantarMagangApplication
    {
    ...
    private function bindServicesThatMustNotRun(): void
    {
